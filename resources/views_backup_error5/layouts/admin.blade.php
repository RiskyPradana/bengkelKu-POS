...
<head>
    ...
    <title>{{ $pageTitle ?? 'Dashboard' }} &mdash; BengkelOS</title>
...
<div class="flex min-h-screen">
        ...
        <nav class="flex-1 px-3 py-5 space-y-5 overflow-y-auto">
            ...
            <div>
                ...
                <div class="space-y-0.5">
                    <a href="{{ route('work-orders') }}" class="nav-link {{ request()->routeIs('work-orders') ? 'active' : '' }}">
                        <span class="nav-icon">&#x1F4CB;</span>
                        <span>Work Order (SPK)</span>
                    </a>
                    <a href="{{ route('pos.cashier') }}" class="nav-link {{ request()->routeIs('pos.cashier') ? 'active' : '' }}">
                        <span class="nav-icon">&#x1F9EE;</span>
                        <span>Kasir &amp; POS</span>
                    </a>
                </div>
        ...
        <div class="px-3 py-3 border-t border-slate-800/70">
            <div class="flex items-center gap-3 px-2 py-2">
                ...
                <div class="flex-1 min-w-0">
                    <div class="text-[13px] font-semibold text-slate-200 truncate">{{ auth()->user()->name ?? 'Admin' }}</div>
    ...
    <div class="flex-1 ml-60 flex flex-col min-h-screen">
        ...
        <header class="sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-slate-200/60 px-6 py-3">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    @if(!empty($pageSub))
                    <p class="text-xs text-slate-400 mt-0.5">{{ $pageSub }}</p>
                    @endif
                </div>
                <div class="flex items-center gap-3">
        ...
        <footer class="px-6 py-3 border-t border-slate-200/60 text-center text-[11px] text-slate-400">
            &copy; {{ date('Y') }} BengkelOS &mdash; Laravel {{ app()->version() }}
        </footer>
    </div>
</div>
