<div class="p-6 space-y-6">
    ...
    <div class="flex gap-1 bg-slate-100 rounded-xl p-1">
        <button type="button" wire:click="$set('activeTab','reminders')"
            class="flex-1 py-2 text-sm font-medium rounded-lg transition-colors {{ $activeTab==='reminders' ? 'bg-white text-amber-600 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
            &#x1F4CB; Daftar Pengingat
        </button>
        <button type="button" wire:click="$set('activeTab','logs')"
            class="flex-1 py-2 text-sm font-medium rounded-lg transition-colors {{ $activeTab==='logs' ? 'bg-white text-amber-600 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
            &#x1F4F1; Log WhatsApp
        </button>
    </div>
    ...
    @if($showCreateModal)
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4" wire:click.self="$set('showCreateModal',false)">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <select wire:model="createVehicleId" class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none">
                        <option value="">-- Pilih Kendaraan --</option>
                        @foreach($allVehicles as $v)
                        <option value="{{ $v->id }}">{{ $v->plate_number }} - {{ $v->customer->name ?? '' }}</option>
                        @endforeach
                    </select>
                    @error('createVehicleId') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="text-sm font-medium text-slate-700 mb-1 block">Tipe Pengingat</label>
                    <select wire:model="createReminderType" class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none">
                ...
                <div>
                    <label class="text-sm font-medium text-slate-700 mb-1 block">Jadwal Kirim</label>
                    <input wire:model="createScheduledAt" type="datetime-local"
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none">
                    @error('createScheduledAt') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="text-sm font-medium text-slate-700 mb-1 block">Pesan Kustom (opsional)</label>
                    <textarea wire:model="createCustomMessage" rows="3" placeholder="Kosongkan untuk pesan otomatis..."
            ...
            <div class="flex gap-3 pt-2">
                <button type="button" wire:click="$set('showCreateModal',false)" class="flex-1 py-2 border border-slate-200 rounded-xl text-sm font-medium text-slate-600 hover:bg-slate-50">Batal</button>
