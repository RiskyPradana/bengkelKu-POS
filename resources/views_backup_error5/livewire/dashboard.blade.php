...
<div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col gap-3">
        ...
        <div class="font-display text-4xl font-bold text-slate-800">{{ $stats['total'] }}</div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col gap-3">
        ...
        <div class="font-display text-4xl font-bold text-amber-500">{{ $stats['pending'] }}</div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col gap-3">
        ...
        <div class="font-display text-4xl font-bold text-blue-500">{{ $stats['in_progress'] }}</div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col gap-3">
        ...
        <div class="font-display text-4xl font-bold text-emerald-500">{{ $stats['completed'] }}</div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col gap-3">
        ...
        <div class="font-display text-4xl font-bold text-violet-500">{{ $stats['paid'] }}</div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col gap-3">
        ...
        <div class="font-display text-4xl font-bold text-rose-500">{{ $stats['total_customers'] }}</div>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div class="relative overflow-hidden rounded-2xl bg-slate-900 p-6 text-white">
        ...
        <div class="relative">
            <div class="text-[10px] font-bold uppercase tracking-widest text-slate-400">Omzet {{ now()->locale('id')->isoFormat('MMMM YYYY') }}</div>
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 p-6 text-white">
        ...
        <div class="relative">
            ...
            <div class="text-xs text-blue-200 mt-2">{{ now()->locale('id')->isoFormat('dddd, D MMMM YYYY') }}</div>
        </div>
        <div class="relative mt-5 flex items-center gap-2">
        ...
        <div class="p-4">
            <div class="flex items-center gap-2 mb-3">
                ...
                <span class="ml-auto font-display text-xl font-bold text-amber-500">
                    {{ isset($kanban['Pending']) ? count($kanban['Pending']) : 0 }}
                </span>
            </div>
            <div class="space-y-2">
                @forelse(($kanban['Pending'] ?? []) as $wo)
                <div class="rounded-xl bg-amber-50 border border-amber-100 p-3">
                    <div class="font-semibold text-[12px] text-slate-700 truncate">{{ $wo->customer?->name ?? '—' }}</div>
                    <div class="text-[11px] text-slate-400 mt-0.5">{{ $wo->vehicle?->plate_number ?? '—' }}</div>
                </div>
                @empty
        ...
        <div class="p-4">
            <div class="flex items-center gap-2 mb-3">
                ...
                <span class="ml-auto font-display text-xl font-bold text-blue-500">
                    {{ isset($kanban['In Progress']) ? count($kanban['In Progress']) : 0 }}
                </span>
            </div>
            <div class="space-y-2">
                @forelse(($kanban['In Progress'] ?? []) as $wo)
        ...
        <div class="p-4">
            <div class="flex items-center gap-2 mb-3">
                ...
                <span class="ml-auto font-display text-xl font-bold text-emerald-500">
                    {{ isset($kanban['Completed']) ? count($kanban['Completed']) : 0 }}
                </span>
            </div>
            <div class="space-y-2">
        ...
        <div class="p-4">
            <div class="flex items-center gap-2 mb-3">
                ...
                <span class="ml-auto font-display text-xl font-bold text-violet-500">
                    {{ isset($kanban['Paid']) ? count($kanban['Paid']) : 0 }}
                </span>
            </div>
            <div class="space-y-2">
                @forelse(($kanban['Paid'] ?? []) as $wo)
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-50">
                @forelse($recent as $i => $wo)
                <tr class="hover:bg-slate-50/60 transition-colors">
                    <td class="px-6 py-3.5">
                        <span class="font-mono-jet text-[11px] text-slate-400">#{{ str_pad($i + 1, 3, '0', STR_PAD_LEFT) }}</span>
                    </td>
                    <td class="px-6 py-3.5">
                        <div class="font-medium text-slate-700">{{ $wo->customer?->name ?? '—' }}</div>
                    </td>
                    ...
                    <td class="px-6 py-3.5">
                        ...
                        @endphp
                        <span class="inline-block text-[11px] font-semibold px-2.5 py-1 rounded-full {{ $badge }}">
                            {{ $statusLabel }}
                        </span>
                    </td>
