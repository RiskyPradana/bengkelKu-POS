<div class="p-6 space-y-6">
    ...
    <div class="flex gap-1 bg-slate-100 rounded-xl p-1">
        ...
        <button type="button" wire:click="$set('activeTab','{{ $key }}')"
            class="flex-1 py-2 text-sm font-medium rounded-lg transition-colors
                {{ $activeTab === $key ? 'bg-white text-amber-600 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
            {!! $label !!}
        </button>
        @endforeach
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-4 border-b border-slate-100 flex items-center gap-3">
            <input wire:model.live="searchStock" type="text" placeholder="Cari produk..."
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-center">
                            <button type="button" wire:click="openAdjust({{ $stock->id }})"
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <select wire:model="adjustType" class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none">
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <select wire:model="transferFromBranch" class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none">
                        <option value="">-- Pilih --</option>
                        @foreach($branches as $b)<option value="{{ $b->id }}">{{ $b->name }}</option>@endforeach
                    </select>
                    @error('transferFromBranch') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="text-sm font-medium text-slate-700 mb-1 block">Ke Cabang</label>
                    <select wire:model="transferToBranch" class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none">
                        <option value="">-- Pilih --</option>
                        @foreach($branches as $b)<option value="{{ $b->id }}">{{ $b->name }}</option>@endforeach
                    </select>
                    @error('transferToBranch') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
            </div>
            <div>
                <label class="text-sm font-medium text-slate-700 mb-1 block">Catatan</label>
                <input wire:model="transferNotes" type="text" placeholder="Opsional..."
                    class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none">
            </div>
            {{-- Item List --}}
            <div class="space-y-2">
                ...
                <div class="flex gap-2 items-center">
                    <select wire:model="transferItems.{{ $idx }}.product_id" class="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none">
                        <option value="">-- Pilih Produk --</option>
                        @foreach($allProducts as $p)<option value="{{ $p->id }}">{{ $p->name }}</option>@endforeach
                    </select>
                    <input wire:model="transferItems.{{ $idx }}.quantity" type="number" min="1" placeholder="Qty" class="w-20 border border-slate-200 rounded-lg px-2 py-2 text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none text-center">
